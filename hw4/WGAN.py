...
class WGAN():
    ...
    def generator(self, sample, is_train):
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            ...
            # TODO: better to remove BN layers in generator
    ...
    def gradient_penalty(self, fake_imgs, true_imgs):
        # uniform sample
        alpha = tf.random_uniform([self.batch_size, 1], minval=0., maxval=1.)
        alpha = tf.tile(tf.reshape(alpha, [-1, 1, 1, 1]), [1, self.image_size[0], self.image_size[1], self.image_size[2]])

        diff = fake_imgs - true_imgs
        sample = true_imgs + alpha*diff
        sample_score = self.discriminator(sample, False)

        gradients = tf.gradients(sample_score, sample)
        slopes = tf.sqrt(tf.reduce_sum(tf.square(gradients), axis=1))
        penalty = self.grad_penalty*tf.reduce_mean((slopes-1)**2)
        return penalty
    ...
    def train(self, imgs_train, labs_train, imgs_test, labs_test):
        ...
        with tf.Session() as sess:
            ...
            for iter in range(self.max_iter):
                _, gen_loss, dis_loss = sess.run([d_op, g_loss, d_loss], \
                ...
                print('Iter ', iter + 1, '\tG_Loss: ', gen_loss, '\tD_Loss: ', dis_loss)
            ...
            coord.request_stop()
            coord.join(threads)
            tf.train.Saver().save(sess, './model/WGAN')
    ...
    def generate(self, path):
        ...
        with tf.Session() as sess:
            ...
            generate = sess.run(fake_imgs, {z: np.random.uniform(-1., 1., size=[gen_num, self.latent_num])})
            generate = (generate+1)/2  # to [0,1]
