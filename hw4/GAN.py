...
class GAN():
    ...
    def train(self, imgs_train, labs_train, imgs_test, labs_test):
        ...
        lr = tf.train.exponential_decay(self.lr_start, self.max_iter, self.epoch_size, self.lr_decay, staircase=True)
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        with tf.control_dependencies(update_ops):
            g_op = tf.train.AdamOptimizer(learning_rate=lr, beta1=0.5) \
                .minimize(g_loss, var_list=g_var)
            d_op = tf.train.AdamOptimizer(learning_rate=lr/5, beta1=0.5) \
